...
/* ════════════════════════════════════════════════════════════════════════
   Globe3D — adapted from Originkit's "Globe" component (originkit.dev/
   components/globe), converted from TS to plain JS for this codebase and
   with a couple of performance additions on top of the original:

     • Pauses its render loop (no rAF, no rotation math) whenever the
       canvas scrolls out of the viewport or the browser tab goes to the
       background, via IntersectionObserver + the visibilitychange event,
       and resumes automatically when it's visible again. The upstream
       component runs its animation loop continuously.
     • Drops the original's per-feature console.log (dev-only noise).

   Everything else — the dot/solid landmass fill, drag-to-rotate physics,
   marker rendering — is unchanged from the source the design was copied
   from. Land geometry comes from a public GeoJSON file fetched at
   runtime (same as upstream); nothing is bundled.
   ════════════════════════════════════════════════════════════════════════ */
...
export default function Globe3D({
  ...
  useEffect(() => {
    ...
    // ── Visibility gating (perf addition, not in the upstream component) ──
    // The render loop below only keeps scheduling itself while the canvas
    // is actually on screen *and* the tab is in the foreground. Scrolling
    // the globe out of view or switching tabs stops the rAF loop entirely
    // instead of spinning it uselessly in the background; the
    // IntersectionObserver/visibilitychange handlers below kick it back
    // into motion the moment either condition is true again.
    ...
    const startAnimation = () => {
      if (animationFrameId === null && isIntersecting && document.visibilityState === "visible") {
        animationFrameId = requestAnimationFrame(animate);
      }
    };
    if (rotationSpeed !== 0) {
